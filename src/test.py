...
class MultiGoals:
    def __init__(self, goalListX, goalListY, retry, map_frame):
        self.sub = rospy.Subscriber('move_base_simple/goal', MoveBaseActionResult, self.statusCB, queue_size=10)
        self.pub = rospy.Publisher('move_base/goal', PoseStamped, queue_size=10)   
        # params & variables
...
if __name__ == "__main__":
    try:    
        ...
        # Get params
        goalListX = rospy.get_param('~goalListX', '[1.1, 1.1]')
        goalListY = rospy.get_param('~goalListY', '[1.2, 3.1]')
